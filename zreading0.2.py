...
class Zreading_Spider:
    ...
    def page_counter(self,myPage):
        # 匹配 "共有<a href="http://www.zreading.cn/page/283" class="extend" title="跳转到最后一页">尾页</a>" 来获取一共有多少页
        myMatch = re.search(r'<a href(.*?)(\d+?)(.*?)title="跳转到最后一页">尾页</a>', myPage, re.S)
        if myMatch:
            print 'OK'
        
        if myMatch:  
            endPage = int(myMatch.group(1))
            print u'爬虫报告：发现博客共有%d页的内容' % endPage
        else:
            endPage = 0
            print u'爬虫报告：无法计算博客内容有多少页！'
        #return endPage
